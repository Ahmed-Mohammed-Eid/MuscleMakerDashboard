...
export default function DefaultPage({ params: { locale } }) {
    ...
    const handleSubmit = async () => {
        ...
        // SET DYNAMIC URL BASED ON THE ROLE
        let url = `${process.env.API_URL}/add/menu/day`;
    ...
    return (
        <>
            <form className={'card flex gap-2 justify-content-end flex-wrap mb-2'} onSubmit={(e) => e.preventDefault()}>
                <Button label={t('selectedDaysList')} severity={'help'} className={'mr-auto'} onClick={() => router.push(`/${locale}/menu/default/selected`)} />
            ...
            <div className={'card mb-0'} dir={isRTL ? 'rtl' : 'ltr'}>
                <div className="flex flex-wrap align-items-center justify-content-between gap-2 mb-4">
                    <h3 className="m-0 uppercase">{t('defaultMenuTitle')}</h3>
