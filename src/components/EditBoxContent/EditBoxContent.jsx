...
export default function EditBoxContent({ boxData, id, locale, isRTL }) {
    const t = useTranslations('editDayBox');
    ...
    function editBoxHandler(e) {
        ...
        axios
            .put(`${process.env.API_URL}/edit/box`, formData, {
                headers: {
    ...
    const getMenuAndTypes = useCallback(
        (types) => {
            setBoxMenu((prevMenu) => {
                // Only update state if the new value is different
                if (JSON.stringify(prevMenu) !== JSON.stringify(types)) {
                    return types;
                }
                return prevMenu;
            });
    ...
    return (
        <form onSubmit={editBoxHandler} dir={isRTL ? 'rtl' : 'ltr'}>
            <div className={'card'}>
            ...
            <DayBoxMenu boxMenu={boxData?.boxMenu} getMenuAndTypes={getMenuAndTypes} />
            ...
            >
                <Button type={'submit'} style={{ width: '100%', textTransform: 'uppercase' }} label={loading ? <ProgressSpinner fill={'#fff'} strokeWidth={'4'} style={{ width: '2rem', height: '2rem' }} /> : t('editButton')} disabled={loading} />
            </div>
        </form>
    );
}
