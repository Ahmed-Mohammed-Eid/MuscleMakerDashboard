...
export default function DayBoxMenu({ getMenuAndTypes, boxMenu }) {
    ...
    // SEND THE TYPES TO THE PARENT ONLY IF TYPES CHANGES
    useEffect(() => {
        if (getMenuAndTypes) {
    ...
    // SET THE BOX MENU
    useEffect(() => {
        if (boxMenu && allMeals.length > 0) {
            const ArrayOfTypes = boxMenu.map((item) => ({
                menuType: item.mealType,
                selectedMenu: item.mealsIds.map((meal) => meal._id),
                MINMAX: item.allowedNumber
            }));

            ArrayOfTypes.forEach((item, index) => {
                const filteredMeals = allMeals.filter((meal) => meal.mealType === item.menuType);
                const menu = filteredMeals.map((meal) => ({
                    value: meal._id,
